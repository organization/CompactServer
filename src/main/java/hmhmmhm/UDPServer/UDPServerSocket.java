...
public class UDPServerSocket {
	...
	public UDPServerSocket(int port, String interfaz) {
		try {
			...
			MainLogger.critical("**** FAILED TO BIND TO " + interfaz + ":" + port + "!");
			MainLogger.critical("Perhaps a server is already running on that port?");
			System.exit(1);
		}
	}
	...
	public DatagramPacket readPacket() throws IOException {
		...
		// MainLogger.getLogger().debug(TextFormat.YELLOW + "In: " +
		// Binary.bytesToHexString(packet.getData(), true));
		return packet;
		/*
	...
	public int writePacket(byte[] data, InetSocketAddress dest) throws IOException {
		// MainLogger.getLogger().debug(TextFormat.AQUA + "Out: " +
		// Binary.bytesToHexString(data, true));
		return this.channel.send(ByteBuffer.wrap(data), dest);

		/*
		 * DatagramPacket packet = new DatagramPacket(data, data.length, dest);
		 * this.socket.send(packet); return packet.getLength();
		 */
	}
